...
actual object MacroCalculator {
    ...
    actual val isInitializing: StateFlow<Boolean> = _isInitializing.asStateFlow()

    private val _isReady = MutableStateFlow(false)
    actual val isReady: StateFlow<Boolean> = _isReady.asStateFlow()

    private val _downloadProgress = MutableStateFlow<Float?>(null)
    actual val downloadProgress: StateFlow<Float?> = _downloadProgress.asStateFlow()
    ...
    actual suspend fun init() = withContext(aiExecutor) {
    ...
    actual suspend fun release(): Unit = withContext(aiExecutor) {
        engine?.close()
        engine = null
    ...
    actual suspend fun generateResponse(prompt: String): String? = withContext(aiExecutor) {
    ...
    actual fun generateResponseStream(prompt: String): Flow<String> = channelFlow {
    ...
    actual fun sendChatMessageStream(prompt: String): Flow<AiStreamItem> = channelFlow {
        withContext(aiExecutor) {
            if (isProcessing.getAndSet(true)) {
    ...
    actual suspend fun summarizeMessages(conversationText: String): String? = withContext(aiExecutor) {
