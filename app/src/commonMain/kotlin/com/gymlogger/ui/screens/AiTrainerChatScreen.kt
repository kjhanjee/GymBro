...
import com.gymlogger.ai.AiStreamItem
...
fun AiTrainerChatScreen(onNavigateBack: () -> Unit) {
    ...
    val trainingContextNames by SettingsRepository.getTrainingContext().collectAsStateWithLifecycle(initialValue = emptyList())
    ...
    LaunchedEffect(Unit) {
        ...
        val historyJson = SettingsRepository.getChatHistory()
    ...
    LaunchedEffect(messages.size) {
        if (messages.isNotEmpty()) {
            ...
            SettingsRepository.setChatHistory(historyJson)
        }
    }
    ...
    suspend fun getSystemPrompt(): String {
        ...
        val targetWeight = SettingsRepository.getTargetWeight().first()
        val goal = SettingsRepository.getGoal().first()
                ...
                actions = {
                    IconButton(
                        onClick = {
                            ...
                            MacroCalculator.startChat()
                            scope.launch {
                                SettingsRepository.setChatHistory("")
                            }
                        }
                    ) {
                        Icon(
                                    ...
                                    DropdownMenuItem(
                                        ...
                                        onClick = {
                                            if (!trainingContextNames.contains(routine.name)) {
                                                scope.launch {
                                                    SettingsRepository.setTrainingContext(trainingContextNames + routine.name
                                                    )
                                                }
                                            }
                                            expanded = false
                        ...
                        androidx.compose.foundation.lazy.LazyRow(
                            ...
                            items(trainingContextNames) { name ->
                                InputChip(
                                    ...
                                    onClick = {
                                        scope.launch {
                                            SettingsRepository.setTrainingContext(trainingContextNames.filter { it != name }
                                            )
                                        }
                                    },
