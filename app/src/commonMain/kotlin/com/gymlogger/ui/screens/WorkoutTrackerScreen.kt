...
fun WorkoutTrackerScreen(
    ...
    DisposableEffect(Unit) {
        WorkoutServiceController.startWorkoutService()
    ...
    val secondsElapsed by WorkoutServiceController.secondsElapsed.collectAsStateWithLifecycle()
    val restSecondsRemaining by WorkoutServiceController.restSecondsRemaining.collectAsStateWithLifecycle()
    val isRestTimerActive by WorkoutServiceController.isRestTimerActive.collectAsStateWithLifecycle()
    ...
    LaunchedEffect(workoutTitle, exercises.size, exercises.sumOf { it.sets.size }, exercises.sumOf { it.sets.count { s -> s.isCompleted } }) {
        ...
        WorkoutServiceController.updateWorkoutStats(workoutTitle, total, completed)
    }
    ...
    fun saveWorkout() {
        scope.launch {
            // Stop service
            WorkoutServiceController.stopWorkoutService()
            ...
            if (workoutSets.isNotEmpty()) {
                ...
                RoutineRepository.completeWorkout(workout)
                RoutineRepository.clearInProgressWorkout()
            }
            onNavigateBack()
        }
    }
                    ...
                    Column {
                        Row(
                            ...
                            if (isRestTimerActive) {
                                ...
                                IconButton(
                                    onClick = { WorkoutServiceController.stopRestTimer() },
                    ...
                    ExerciseTrackerItem(
                        ...
                        onToggleSetCompleted = { setIndex, completed ->
                            ...
                            if (completed && exerciseState.restTime > 0) {
                                WorkoutServiceController.startRestTimer(exerciseState.restTime)
                            }
                        },
